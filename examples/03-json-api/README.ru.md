...
> **Заметка перед чтением кода:** каждый эндпоинт здесь строит JSON через
> `#impl(Serialize)` + `ServerResponse.json` — это канон — **кроме**
> `GET /todos`, который склеивает массив вручную строкой. Эта единственная
> функция — узкий обход открытой дыры кодогена (`Vec[T Serialize]`, см.
> «Что этот пример обходит» ниже), **не** образец для копирования.
