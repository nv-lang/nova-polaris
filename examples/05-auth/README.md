...
from a public one via `nest` + `.use()`, a `/login` endpoint that mints a
...
- Layer `require_jwt` at the TOP level instead of nesting `/private` — see
  what happens to `/public` (hint: `.use()` only wraps routes registered
...
- [`docs/middleware.md`](../../docs/middleware.md) — `nest` + `.use()` interaction
