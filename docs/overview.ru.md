...
схождения `IntoResponse` для возвращаемых значений обработчиков, chi-стиль
композиции промежуточных обработчиков, набор готовых «батареек» (CORS, сжатие, логирование,
...
поверхность обработчиков; пользователь Polaris обычно не импортирует `http`
напрямую (он приходит транзитивно) — кроме случаев, когда нужно назвать
`StatusCode`/`HttpError` в сигнатуре собственного обработчика, как в примерах
ниже.

Axum стоит поверх `hyper` так же, как Polaris стоит поверх `http` — ядро
владеет байтами на проводе, Polaris — маршрутизацией/обработчиками/промежуточными обработчиками/serve.
...
«mock-first»: весь конвейер маршрутизация/обработчик/промежуточный обработчик исчерпывающе
...
импортирует `Router`/маршрутизацию (нижний слой не зависит от верхнего) — вместо
