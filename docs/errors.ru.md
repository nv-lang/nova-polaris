...
До хендлера доходят два разных канала отказа, и Polaris намеренно
...
- **Программные ошибки и по-настоящему неожиданные отказы** (assert,
  выход за границы, ускользнувший `Fail`-throw из тела хендлера) — это
...
```
  ...
  **Только сообщение, намеренно** (D437): полная диагностика рантайма
  ...
  трассе из `Supervisor`-хендлера, поэтому recover-500 честно логирует то,
