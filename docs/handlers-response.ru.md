# Обработчики, запросы и ответы

[English](handlers-response.md) | **Русский**

Эта страница покрывает модель запрос/ответ, с которой работает обработчик:
сам `Handler`, чтение `ServerRequest`, построение `ServerResponse`, протокол
схождения `IntoResponse`, позволяющий обработчику возвращать не только голый
...
Тело запроса к моменту, когда его видит обработчик, всегда полностью
...
Всё, что обработчик может вернуть, сходится сюда — по духу тот же `IntoResponse`
...
рядом с обработчиком: пишем fallible-логику как обычную функцию, возвращающую
...
зарегистрировать». Чтобы обработчик вместо этого принимал голое типизированное
...
обработчик формы `Router.@post_typed_h[T](path, h fn(T) -> ServerResponse)`
